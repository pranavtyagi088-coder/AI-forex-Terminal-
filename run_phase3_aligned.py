...
# ═══════════════════════════════════════════════════════════════
# STEP 1: Align frontend/e2e/p1_runtime.spec.ts with Backend Schemas
# ═══════════════════════════════════════════════════════════════
log("STEP 1: Writing Institutional E2E Specs (p1_runtime.spec.ts)...", "\033[93m")
...
test.describe('Phase P1 Live Runtime & Risk Cockpit Verification', () => {
  ...
  test('P1-01: Terminal Cockpit Hydration & Live WS Stream Verification', async ({ page }) => {
    ...
    // Verify Telemetry or WS status is visible
  ...
  test('P1-02: Pre-Flight Trade Gate Evaluation (9/9 Gates Approved)', async ({ page }) => {
    // Direct REST evaluation against backend schema
  ...
  test('P1-03: Excessive Spread Rejection (Gate 3 Veto)', async ({ page }) => {
    const response = await page.request.post('http://127.0.0.1:8000/api/trades/pre-flight-check', {
      ...
      data: {
        ...
        account_equity: 100000.0,
        risk_per_trade_pct: 1.0,
        current_spread_pips: 5.0 // Excessive spread (> 2.5 pips max limit)
      }
    });
  ...
  test('P1-04: Circuit Breaker Intercept & Kill Switch Gate Intercept', async ({ page }) => {
    ...
    // 2. Toggle Circuit Breaker to ON (Trip Kill Switch)
    ...
    // 4. Reset Circuit Breaker back to NORMAL
...
log("✅ e2e/p1_runtime.spec.ts aligned with backend schema", "\033[92m")
...
# ═══════════════════════════════════════════════════════════════
# STEP 3: Start Services for Playwright E2E
# ═══════════════════════════════════════════════════════════════
log("STEP 3: Launching services for E2E...", "\033[93m")
...
try:
    ...
    print("\n" + "═"*75)
    print("\033[92m🎉 PHASE 3 PLAYWRIGHT E2E AUTOMATION COMPLETE & VERIFIED 100%!\033[0m")
